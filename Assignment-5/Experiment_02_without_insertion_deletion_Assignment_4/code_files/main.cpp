...
int main() {
    printf("\n\n========================================\n");
    printf("EXPERIMENT 02 - A4 Mover Only\n");
    printf("========================================\n");
    ...
    for (int g = 0; g < num_grids; g++) {
        ...
        char fname[64];
        sprintf(fname, "exp2_a4mover_grid%d.csv", g + 1);
        ...
        if (!mesh_value || !points || !points_init) {
            fprintf(stderr, "malloc failed in a4 mover run\n");
            return 1;
        }
        ...
        for (int tc = 0; tc < num_thread_sets; tc++) {
            ...
            for (int iter = 0; iter < Maxiter; iter++) {
                ...
                if (nthreads == 1)
                    ...
                    mover_parallel(points, dx, dy, nthreads);
